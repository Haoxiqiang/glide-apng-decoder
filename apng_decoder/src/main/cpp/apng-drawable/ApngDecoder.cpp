...
namespace apng_drawable {
    ...
    inline void blendOver(
        ...
        for (uint32_t j = 0; j < height; ++j) {
            ...
            uint8_t sourceAlpha;
            for (uint32_t i = 0;
                 i < width; ++i, sp += CHANNEL_4_BYTE_SIZE, dp += CHANNEL_4_BYTE_SIZE) {
                sourceAlpha = sp[3];
                if (sourceAlpha == ALPHA_OPAQUE) {
                    ...
                    if (dp[3] != ALPHA_TRANSPARENT) {
                        ...
                        int32_t al =
                                ...
                                (ALPHA_OPAQUE - sourceAlpha) * (ALPHA_OPAQUE - dp[3]);
    ...
    std::unique_ptr<ApngImage> ApngDecoder::decode(
        ...
        png_structp png_ptr = png_create_read_struct(PNG_LIBPNG_VER_STRING, nullptr, nullptr,
                                                     nullptr);
        ...
        for (uint32_t i = 0; i < frames; ++i) {
            ...
            auto duration =
                    static_cast<size_t>(std::lround(
                            static_cast<float>(delay_num) / delay_den * 1000.F));
            ...
            if (blend_op == PNG_BLEND_OP_OVER) {
                blendOver(rows_frame.get(), rows_buffer.get(), x_offset, y_offset, frame_width,
                          frame_height);
